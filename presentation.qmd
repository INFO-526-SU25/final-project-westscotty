...
``` r
aggregated_data <- vesuvius_data_filtered %>%   group_by(year) %>%   summarise(mean_duration_magnitude_md = mean(duration_magnitude_md),             mean_depth_km = mean(depth_km))
```
...
``` r
vesuvius_data <- vesuvius_data |>     mutate(         date = str_replace_all(time, "T", " "),         date = str_replace_all(date, "Z", ""),         date = as.POSIXct(date, format = "%Y-%m-%d %H:%M:%S")     )
```
...
:::::: columns
::: {.column width="65%"}
![](images/by_year_month.png)

Time plot of the `duration_magnitude` and `dept_km` for seismic event down-sampled.
:::
...
:::::: columns
::: {.column width="65%"}
![This plot shows the "trend" of the `duration_magnitude_md`, the spread of the `duration_magnitude_md` and breaks down the frequency of the duration_magnitude for each year.](images/duration_magnitude_error_binned.png)
:::
