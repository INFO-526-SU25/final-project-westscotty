...
::: {style="font-size: 0.75em; line-height: 1.25"}
<details>
  <summary style="color: black;">The first step was to remove bad (or missing) data by dropping rows with NA in `latitude`, `longitude`, `depth_km`, and `duration_magnitude_md`</summary>
...
<details>
  <summary style="color: black;">The first step was to down-sample the raw depth data into levels only depth boundaries spanning 0.5 km bands, creating a new variable titled `depth_boundary_0.5km`, plotting in ascending depth order meaning that deeper values will appear first</summary>
...
<details>
  <summary style="color: black;">A size re-scaling for points was created from this new depth layer variable</summary>
...
<details>
  <summary style="color: black;">Initially a visualization was created to simply view lat/long axes with points that were colored by the `duration_magnitude_md` and a transparency difference based on `depth_km` (see Initial View on next slide)</summary>
...
<details>
  <summary style="color: black;">Next the data was plotted as histograms of the `duration_magnitude_md`, but faceted by the newer `depth_boundary_0.5km` variable, showing the groupings of points clearer, as well as their distributions across the magnitude scale</summary>
...
<details>
  <summary style="color: black;">After learning from the histogram, the initial plot was updated to plot the points with varying sizes based on `depth_boundary_0.5km`, with contour lines of the elevation in the same region, an accompanying animated plot followed it</summary>
...
:::::: columns
::: {.column width="65%"}
![First pass at plotting depth and mangitude on lat/lon](images/original_vesuvius_plot.png){width="100%"}
:::
